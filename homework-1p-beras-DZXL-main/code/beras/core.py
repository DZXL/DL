...
class Diffable(Callable, Weighted):
    ...
    def compose_input_gradients(self, J=None):
        """
        Compose (multiply) the inputted cumulative jacobian with the input jacobian for the layer.
        ...
        for upstream_jacobian in J:
            ...
            for layer_input, inp_grad in zip(self.inputs, self.get_input_gradients()):
                ...
                for sample in range(batch_size):
                    ...
                    try:
                        j_wrt_lay_inp[sample] = s_grad @ inp_grad[sample]
    ...
    def compose_weight_gradients(self, J=None) -> list[Tensor]:
        """
        Compose (multiply) the inputted cumulative jacobian with the weight jacobian for the layer.
        Implemented with batch-level vectorization.
